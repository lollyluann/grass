...
class CUBDataset(ConfounderDataset):
    ...
    def __init__(
        ...
        if override_groups_file and override_groups_file!="None":
            print("overriding group label files with", override_groups_file)
            self.glabel_array = pd.read_csv(os.path.join(self.data_dir, override_groups_file), index_col=False)['clustered_idx'].to_numpy()
        else:
