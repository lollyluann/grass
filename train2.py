...
def run_epoch(
    ...
    with torch.set_grad_enabled(is_training or data_grad):
        ...
        for batch_idx, batch in enumerate(prog_bar_loader):
            ...
            if data_grad:
                for i, pt in enumerate(x):
                    ...
                    loss2 = loss_computer.criterion(ptpred, y[i:i+1])
                    loss2.backward()
                    last_grad = full_detach(model.fc.weight.grad)
                    last_grad_bias = full_detach(model.fc.bias.grad)
                    last_grad = np.append(last_grad, np.reshape(last_grad_bias, (2, -1)), axis=1)
        ...
        if data_grad:
            ...
            all_last_grads = np.stack(all_last_grads, axis=0)
            #all_penult_grads = np.array(all_penult_grads)
            save_dir = "/".join(csv_logger.path.split("/")[:-1])
            if os.path.exists(os.path.join(save_dir, f"last_fc_grads_epoch_{epoch-50}.npy")):
                os.remove(os.path.join(save_dir, f"last_fc_grads_epoch_{epoch-50}.npy"))
            np.save(
                os.path.join(save_dir, f"last_fc_grads_val_epoch_{epoch}.npy"),
                all_last_grads)
            #np.save(
            #    os.path.join(save_dir, f"penult_fc_grads_epoch_{epoch}.npy"),
            #    all_penult_grads)
            print("gradients have size", all_last_grads.shape)
            print(f"Saved gradients to last/penult_fc_grads_epoch_{epoch}.npy")
